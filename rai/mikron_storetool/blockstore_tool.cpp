...
void rai::blockstore_tool::debug_pending (boost::filesystem::path data_path)
{
	...
	for (auto i (node.node->store.pending_begin (transaction)), n (node.node->store.pending_end ()); i != n; ++i, ++cnt)
	{
		...
		std::cout << std::endl << "Pending " << cnt << " " << amount.format_balance (rai::Mxrb_ratio, 10, true) << " " << " from " << info.source.to_account () << " to " << key.account.to_account () << " hash " << key.hash.to_string () << std::endl;
	}
	...
	for (auto i (node.node->store.latest_begin (transaction)), n (node.node->store.latest_end ()); i != n; ++i, ++cnt_acc)
	{
		...
		while (block_hash.number () != 0)
		{
			...
			if (block->type () == rai::block_type::state)
			{
				...
				rai::state_block_subtype subtype = rai::state_block_subtype_helper::get_subtype (node.node->ledger, transaction, *state_block);
				if (subtype == rai::state_block_subtype::send)
				{
					...
					{
						// send to self
						std::cout << "Send to self: " << " " << state_block->hashables.account.to_account () << " " << block_hash.to_string () << " " << std::endl;
					}
				}
			}
